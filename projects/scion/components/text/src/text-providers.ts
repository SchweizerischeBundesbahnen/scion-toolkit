...
import {SciTextProviderFn, Translatable} from './text.provider';
...
export class TextProviders {
  ...
  public provide(translatable: Translatable | undefined | null, options?: {params?: Record<string, unknown> | Map<string, unknown>; injector?: Injector}): Signal<string | undefined | null> {
    ...
    const {key, params} = parseTranslatable(translatable as `%${string}`, {params: options?.params});
 ...
 */
function parseTranslatable(translationKey: `%${string}`, options?: {params?: Record<string, unknown> | Map<string, unknown>}): {key: string; params: Map<string, string>} {
  const {key, params} = /^%(?<key>[^;]+)(;(?<params>.*))?$/.exec(translationKey)!.groups!;
  return {
    key: key!,
    params: new Map<string, string>([
      // Append parsed params from translatable.
      ...parseMatrixParams(params),
      // Append params from options.
      ...[...Maps.coerce(options?.params)].map(([key, value]) => [key, `${value}`] satisfies [string, string]),
    ]),
  };
}
