 ...
 *
 * Provided context is applied to menu locations and contributions in the scope of the injector calling this function.
  ...
  return [
    ...
    },
    {
      ...
      useFactory: (): SciMenuContextProviderFn[] => {
        ...
        // Wrap each function to execute within the providing injection context.
        const injector = inject(Injector);
        return [
          ...inject(SCI_MENU_CONTEXT_PROVIDERS, {skipSelf: true, optional: true}) ?? [], // providers registered in parent injectors
          ...inject(SCI_MENU_CONTEXT_PROVIDER).map(fn => () => runInInjectionContext(injector, fn)), // providers registered in this injector
        ];
      },
    },
  ];
 ...
 *
 * Provided tokens are available for dependency injection for contributions registered in the scope of the calling injector.
 ...
 *
 * Provided accelerator targets are available to menu locations in the scope of the injector calling this function.
    ...
    },
    {
      ...
      useFactory: (): SciMenuAcceleratorTargetProviderFn[] => {
        ...
        return [
          ...
          ...inject(SCI_MENU_ACCELERATOR_TARGET_PROVIDER).map(fn => () => runInInjectionContext(injector, fn)), // providers registered in this injector
        ];
      },
    },
  ];
 ...
 */
export type SciMenuContextProviderFn = () => MaybeSignal<Map<string, unknown>>;
 ...
 * The function can call `inject` to get any required dependencies.
 */
export type SciMenuAcceleratorTargetProviderFn = () => MaybeSignal<MaybeArray<Element | ElementRef<Element>> | undefined>;
