    ...
    },
    {
      ...
      useFactory: (): SciMenuContextProviderFn[] => {
        ...
        return [
          ...
          ...inject(SCI_MENU_CONTEXT_PROVIDER).map(fn => (callingInjector: Injector) => runInInjectionContext(injector, () => fn(callingInjector))),
        ];
      },
    },
  ];
    ...
    },
    {
      ...
      useFactory: (): SciMenuInjectionContextProviderFn[] => {
        ...
        return [
          ...
          // Collect providers registered in this injector.
          // Wrap function to execute in the injector of this injector, not in the calling context.
          ...inject(SCI_MENU_INJECTION_CONTEXT_PROVIDER).map(fn => (context: Map<string, unknown>) => runInInjectionContext(injector, () => fn(context))),
        ];
      },
    },
  ];
    ...
    },
    {
      ...
      useFactory: (): SciMenuAcceleratorTargetProviderFn[] => {
        ...
        return [
          ...
          // Collect providers registered in this injector.
          // Wrap function to execute in the injector of this injector, not in the calling context.
          ...inject(SCI_MENU_ACCELERATOR_TARGET_PROVIDER).map(fn => (callingInjector: Injector) => runInInjectionContext(injector, () => fn(callingInjector))),
        ];
      },
    },
  ];
