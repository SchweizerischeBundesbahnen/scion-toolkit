...
export abstract class SciMenuService {
  ...
  /**
   * TODO [menu] JSDoc not yet done! Only for Inspiration!
   *
   * Opens a menu with menu items contributed to the specified location.
   *
   * The menu is positioned relative to the specified anchor, vertically or horizontally.
   * The anchor can be a DOM element, a coordinate, or a mouse event.
   *
   * A menu can define a context, a key-value map describing its environment. Contexts are hierarchical.
   * A Context can be defined at the injector level using {@link provideMenuContextProvider}, enabling a
   * specific context for menus it the injector scope, such as per component or route. Child contexts
   * inherit values from parent contexts, can augment parent contexts, override or clear inherited context values.
   *
   * The context of a menu is available to contributions, enabling context-specific contribution and handling.
   * A contribution is eligible for a location only if the location's context fully
