...
export interface SciMenuContributionOptions {
   ...
   *
   * The contribution is automatically disposed when the passed injector is destroyed.
   */
  injector?: Injector;
   ...
   *
   * Used by frameworks integrating the SCION Menu API. Applications should not use this property.
   */
  metadata?: {[key: string]: unknown};

  /**
   * Specifies a point in time (instant) used to maintain a stable contribution order when the contribution is "replaced", e.g., on context change.
   *
   * Used by frameworks integrating the SCION Menu API. Applications should not use this property.
   */
  contributionInstant?: number;
 ...
 * The menu context is passed as the second argument, which may be required if contributing from elsewhere in the application outside the menu's scope.
 *
 ...
 * The toolbar context is passed as the second argument, which may be required if contributing from elsewhere in the application outside the toolbar's scope.
 *
 * The function runs within a reactive context, running again when tracked signals change.
  * Most menu item properties accept a value or signal. For properties that can change, prefer setting the property as a signal over tracking it manually to prevent this function from re-running.
 *
 ...
 * The menubar context is passed as the second argument, which may be required if contributing from elsewhere in the application outside the menubar's scope.
 *
 * The function runs within a reactive context, running again when tracked signals change.
  * Most menu item properties accept a value or signal. For properties that can change, prefer setting the property as a signal over tracking it manually to prevent this function from re-running.
 *
