...
export function ɵinstallMenuAccelerators(menuItems: SciMenuItemLike[], options?: ɵSciMenuAcceleratorOptions): Disposable {
  ...
  merge(...targets.map(target => fromEvent<KeyboardEvent>(target, 'keydown')))
    .pipe(
 ...
 */
function matchesAccelerator(event: KeyboardEvent, accelerator: SciKeyboardAccelerator): boolean {
