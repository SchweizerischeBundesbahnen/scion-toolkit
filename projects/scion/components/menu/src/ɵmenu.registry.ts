...
export class ɵSciMenuRegistry implements SciMenuRegistry, SciMenuAdapter {
  ...
  private readonly _contributions = new Map<`menu:${string}` | `toolbar:${string}` | `menubar:${string}`, WritableSignal<SciMenuContribution[]>>();
  ...
  public menuItems(location: Signal<`menu:${string}` | `toolbar:${string}` | `menubar:${string}`>, context: Signal<Map<string, unknown>>, options: {injector?: Injector; metadata?: {[key: string]: unknown}}): Signal<SciMenuItemLike[]> {
     ...
     */
    function federateMenuItems(menuItem: SciMenuItemLike, context: Map<string, unknown>, options?: {metadata?: {[key: string]: unknown}}): SciMenuItemLike {
 ...
 */
function parseMenuLocationType(location: `menu:${string}` | `toolbar:${string}` | `menubar:${string}`): 'menu' | 'toolbar' | 'menubar' {
  return /^(?<type>(menu|toolbar|menubar)):(?<name>.+)$/.exec(location)!.groups!['type'] as 'menu' | 'toolbar' | 'menubar';
}
...
export interface CacheEntry {
  ...
  /**
   * Reference count of consumers using this cache entry.
   */
  refCount: number;
  /**
