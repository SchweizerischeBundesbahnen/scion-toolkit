...
export interface SciMenuItem {
  ...
  onSelect?: () => Promise<boolean>; // only for toolbar or menu items with a `onSelect` handler (e.g., toolbar button, toolbar split button, menu item)
  menu?: { // only for toolbar or menu items with a menu (e.g., toolbar split button, menu item for a submenu)
    name?: `menu:${string}`;
    width?: string;
    minWidth?: string;
    maxWidth?: string;
