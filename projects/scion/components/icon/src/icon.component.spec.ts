...
import {SciIconFontGlyphComponent} from './scion-icon-provider';
...
describe('IconComponent', () => {
  ...
  it('should render icon', async () => {
    ...
    await expectIcon(fixture).toDisplay({component: SpecIcon1Component, innerText: 'icon 1'});

    // Render icon-2.
    fixture.componentInstance.icon.set('icon-2');
    await new Promise(resolve => requestAnimationFrame(() => resolve(undefined)));
    await expectIcon(fixture).toDisplay({component: SpecIcon2Component, innerText: 'icon 2'});
  });
  ...
  it('should render icon with input', async () => {
    ...
    // Render icon-1.
    fixture.componentInstance.icon.set('icon-1');
    await expectIcon(fixture).toDisplay({component: SpecIconComponent, innerText: 'icon-1'});

    // Render icon-2.
    fixture.componentInstance.icon.set('icon-2');
    await expectIcon(fixture).toDisplay({component: SpecIconComponent, innerText: 'icon-2'});
  });
  ...
  it('should render icon with changed input ', async () => {
    ...
    await expectIcon(fixture).toDisplay({component: SpecIconComponent, innerText: 'icon - value'});

    // Render icon.
    inputSignal.set('value 1');
    await expectIcon(fixture).toDisplay({component: SpecIconComponent, innerText: 'icon - value 1'});

    // Change input.
    inputSignal.set('value 2');
    await expectIcon(fixture).toDisplay({component: SpecIconComponent, innerText: 'icon - value 2'});
  });
  ...
  it('should render empty icon if no ligature', async () => {
    ...
    await expectIcon(fixture).toDisplay({innerText: '', component: null});
  });
  ...
  it('should destroy icon when removing ligature', async () => {
    ...
    // Render icon.
    fixture.componentInstance.icon.set('icon');
    await expectIcon(fixture).toDisplay({component: SpecIconComponent, innerText: 'icon'});
    const iconComponent = fixture.debugElement.query(By.directive(SpecIconComponent)).componentInstance as SpecIconComponent;
    expect(iconComponent.destroyed).toBeFalse();

    // Remove icon.
    fixture.componentInstance.icon.set(undefined);
    await expectIcon(fixture).toDisplay({component: null, innerText: ''});
    expect(iconComponent.destroyed).toBeTrue();
  });
  ...
  it('should support multiple icon providers', async () => {
    ...
    await expectIcon(fixture).toDisplay({component: SpecIcon1Component, innerText: 'icon 1 [provider=1]'});

    // Render icon-b (provider 1 and provider 2).
    fixture.componentInstance.icon.set('icon-b');
    await expectIcon(fixture).toDisplay({component: SpecIcon2Component, innerText: 'icon 2 [provider=1]'});

    // Render icon-c (provider 2).
    fixture.componentInstance.icon.set('icon-c');
    await expectIcon(fixture).toDisplay({component: SpecIcon4Component, innerText: 'icon 4 [provider=2]'});
  });
  ...
  it('should call icon provider in injection context', async () => {
    ...
    // Render icon.
    fixture.componentInstance.icon.set('icon');
    await expectIcon(fixture).toDisplay({component: MaterialIconComponent, innerText: 'icon'});
  ...
  it('should inherit font-size from location', async () => {
    ...
    expect(getComputedStyle(iconComponent).fontSize).toEqual('14px');
  });
  ...
  it('should inherit font-size from \'--sci-icon-size\' CSS variable', async () => {
    ...
    // Expect font size to be var(--sci-icon-size, 1em).
    const iconComponent = fixture.debugElement.query(By.directive(SpecIconComponent)).nativeElement as HTMLElement;
    expect(getComputedStyle(iconComponent).fontSize).toEqual('12px');
  });
});
...
describe('Built-in Icon Providers', () => {
  ...
  it('should render Material icon if no providers', async () => {
    ...
    await expectIcon(fixture).toDisplay({innerText: 'ligature', component: MaterialIconComponent});
  });
  ...
  it('should render Material icon', async () => {
    ...
    await expectIcon(fixture).toDisplay({
      component: MaterialIconComponent, innerText: 'icon',
    });

    // Expect icon to have the Material icon font.
    const materialIconComponent = fixture.debugElement.query(By.directive(MaterialIconComponent)).nativeElement as HTMLElement;
    expect(getComputedStyle(materialIconComponent).fontFamily).toEqual('"Material Symbols Rounded", "Material Symbols Outlined", "Material Symbols Sharp", "Material Icons", "Material Icons Outlined", "Material Icons Round", "Material Icons Sharp"');
  });
  ...
  it('should render SCION icon', async () => {
    ...
    await expectIcon(fixture).toDisplay({component: SciIconFontGlyphComponent, innerText: 'close'});

    // Expect icon to have the SCION icon font.
    const scionIconComponent = fixture.debugElement.query(By.directive(SciIconFontGlyphComponent)).nativeElement as HTMLElement;
    expect(getComputedStyle(scionIconComponent).fontFamily).toEqual('scion-icons');
  });
  ...
  it('should render custom icon', async () => {
    ...
    await expectIcon(fixture).toDisplay({component: SpecCustomIconComponent, innerText: 'Custom Icon for "icon"'});
...
/**
 * Asserts the given fixture to display the specified icon.
 */
function expectIcon(fixture: ComponentFixture<unknown> | DebugElement): {toDisplay(expected: ExpectedIcon): Promise<void>} {
  const debugElement = fixture instanceof ComponentFixture ? fixture.debugElement : fixture;

  // Use fake assertion to avoid "SPEC HAS NO EXPECTATIONS" warning.
  expect(true).toBe(true);

  return {
    toDisplay: async expected => {
      await retryOnError(() => {
