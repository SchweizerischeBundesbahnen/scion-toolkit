...
export class TablePagePO {
  ...
  public async addColumn(options: ColumnOptions): Promise<void> {
    ...
    if (options.minWidth !== undefined) {
      await this._properties.locator('form input.e2e-column-min-width').fill(options.minWidth.toString());
    }
    if (options.maxWidth !== undefined) {
      await this._properties.locator('form input.e2e-column-max-width').fill(options.maxWidth.toString());
    }
