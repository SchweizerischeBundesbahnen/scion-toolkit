...
test.describe('sci-sashbox', () => {
  ...
  test.describe('Sash Animation', () => {
    ...
    test('should not disable inner Angular animations', async ({page, consoleLogs}) => {
      ...
      await waitUntilStable(() => consoleLogs.get().length);
      await expect.poll(() => consoleLogs.get({message: '[SashContentComponent][sash-4] resize'}).length).toBeGreaterThan(10);

      // Remove sash.
      await sashboxPage.setVisible('sash-4', false);

      await waitUntilStable(() => consoleLogs.get().length);
      consoleLogs.clear();

      await test.step('Opening sash with animation enabled', async () => {
        await sashboxPage.enableAnimation('sash-4', true);
        await sashboxPage.setVisible('sash-4', true);
      ...
      await test.step('Opening sash with animation disabled', async () => {
        await sashboxPage.enableAnimation('sash-4', false);
        await sashboxPage.setVisible('sash-4', true);
