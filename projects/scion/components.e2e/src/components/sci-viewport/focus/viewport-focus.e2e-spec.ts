...
test.describe('sci-viewport/focus', () => {
  ...
  test('should focus elements of slotted content via sequential keyboard navigation', async ({page}) => {
    ...
    expect(await focusPO.isInputActive('before-viewport')).toBe(true);

    // tab to input field inside viewport
    await focusPO.tab();
    expect(await focusPO.isInputActive('inside-viewport')).toBe(true);

    // tab to input field after viewport
    await focusPO.tab();
    expect(await focusPO.isInputActive('after-viewport')).toBe(true);
