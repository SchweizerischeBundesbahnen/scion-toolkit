 ...
 *
 * By default, the Observable uses the native {ResizeObserver} [1] if supported by the user agent [2], or falls back to listening for resize events on
 * a hidden HTML `<object>` element. You can, however, override this strategy to never use the native `ResizeObserver` by setting the global flag
 * `FromDimension.defaults.useNativeResizeObserver` to `false`, or override it locally by passing an options object when constructing the `Observable`.
 *
 * ## Note about HTML `<object>`:
 ...
 * The Web API draft, however, is still work in progress and support limited to Google Chrome and Opera [2].
 *
 * ## Links:
