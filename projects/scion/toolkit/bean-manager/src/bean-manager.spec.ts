...
describe('BeanManager', () => {
  ...
  it('should not destroy the referenced bean when its alias is destroyed [useExisting]', () => {
    ...
    const actualBean = Beans.get(Bean);
    const alias = Beans.get(Alias);
    expect(actualBean).toBe(alias as any); // eslint-disable-line @typescript-eslint/no-unsafe-argument
    ...
    expect(Beans.get(Alias)).toEqual('some-other-bean' as any); // eslint-disable-line @typescript-eslint/no-unsafe-argument
  });
  ...
  it('should register initializers in the configured runlevel (only if not specified by the initializer)', async () => {
    ...
    Beans.registerInitializer({
      useFunction: () => Promise.resolve(void (log.push('initializer runlevel 0'))),
      runlevel: 0,
    });
    Beans.registerInitializer({
      useFunction: () => Promise.resolve(void (log.push('initializer runlevel 1'))),
      runlevel: 1,
    });
    Beans.registerInitializer({
      useFunction: () => Promise.resolve(void (log.push('initializer runlevel 2'))),
      runlevel: 2,
    });
    Beans.registerInitializer({
      useFunction: () => Promise.resolve(void (log.push('initializer runlevel 3'))),
      runlevel: 3,
    });
    Beans.registerInitializer({
      useFunction: () => Promise.resolve(void (log.push('initializer-1 (no runlevel specified)'))),
    });
    Beans.registerInitializer({
      useFunction: () => Promise.resolve(void (log.push('initializer-2 (no runlevel specified)'))),
    });
    Beans.registerInitializer({
      useFunction: () => Promise.resolve(void (log.push('initializer-3 (no runlevel specified)'))),
    });
  ...
  it('should immediately construct eager bean when registered in an initializer that is bound to a higher runlevel than the "eager bean construction runlevel"', async () => {
    ...
    Beans.registerInitializer({
      useFunction: () => Promise.resolve(void (Beans.register(Bean1, {eager: true}))),
      runlevel: eagerBeanConstructRunlevel - 1,
    });
    Beans.registerInitializer({
      useFunction: () => Promise.resolve(void (Beans.register(Bean2, {eager: true}))),
      runlevel: eagerBeanConstructRunlevel,
    });
    Beans.registerInitializer({
      useFunction: () => Promise.resolve(void (Beans.register(Bean3, {eager: true}))),
      runlevel: eagerBeanConstructRunlevel + 1,
    });
  ...
  it('should not construct eager beans when an initializer rejects', async () => {
    ...
    Beans.registerInitializer(() => Promise.reject(new Error('initializer rejected')));
  ...
  it('should construct eager beans after running all initializers', async () => {
    ...
    Beans.registerInitializer(() => Promise.resolve(void (log.push('initializer-1 [no runlevel specified]'))));
    Beans.registerInitializer({
      useFunction: () => Promise.resolve(void (log.push('initializer-2 [runlevel 5]'))),
      runlevel: 5,
    });
    Beans.registerInitializer(() => Promise.resolve(void (log.push('initializer-3 [no runlevel specified]'))));
  ...
  it('should construct eager beans in the runlevel specified', async () => {
    ...
    Beans.registerInitializer(() => Promise.resolve(void (log.push('initializer-1 [no runlevel specified]'))));
    Beans.registerInitializer({
      useFunction: () => Promise.resolve(void (log.push('initializer-2 [runlevel 6]'))),
      runlevel: 6,
    });
    Beans.registerInitializer(() => Promise.resolve(void (log.push('initializer-3 [no runlevel specified]'))));
  ...
  it('should unregister initializers when destroying the bean manager', async () => {
    ...
    Beans.registerInitializer(() => Promise.resolve(void (initializerInvoked = true)));
  ...
  it('should reject the \'start\' Promise when an initializer rejects', async () => {
    Beans.registerInitializer(() => Promise.resolve());
    Beans.registerInitializer(() => Promise.reject(new Error()));
    Beans.registerInitializer(() => Promise.resolve());
  ...
  it('should allow registration of a function initializer (useFunction)', async () => {
    ...
    Beans.registerInitializer({useFunction: () => Promise.resolve(void (initializerInvoked = true))});
    await Beans.start();
