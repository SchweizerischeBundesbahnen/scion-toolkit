...
export default class SciToolbarPageComponent {
  ...
  private contributeToolbar(): void {
    ...
    contributeMenu('toolbar:demo', toolbar => toolbar
      .addToolbarMenu({label: 'File', disabled: false, cssClass: 'file-menu'}, menu => menu
      ...
      .addToolbarSplitControl({component: InputToolbarControlComponent, bindings: [inputBinding('placeholder', signal('Enter to filter'))]}, menu => menu
        .addMenuItem({label: 'Option 1', onSelect})
        .addMenuItem({label: 'Option 2', onSelect})
        .addMenuItem({label: 'Option 3', onSelect}),
      )
      .addGroup(group => group
      ...
      .addToolbarMenu({icon: 'palette', visualMenuIndicator: true, menu: {filter: true}}, menu => menu
        .addGroup(group => group
          .addMenuItem({label: 'Bold', checked: bold, onSelect: () => bold.update(bold => !bold)})
          .addMenuItem({label: 'Italic', accelerator: {ctrl: true, key: 'U'}, actions: toolbar => toolbar.addToolbarButton({icon: 'home', onSelect}), onSelect: () => italic.update(italic => !italic)})
      ...
      .addToolbarMenu({icon: customToolbarIconComponent, visualMenuIndicator: false, menu: {filter: true, maxHeight: '300px'}}, menu => {
          ...
          menu.addMenuItem({
            ...
            actions: actions => actions
              .addToolbarMenu({icon: 'scion.more_vertical', visualMenuIndicator: false}, menu => menu
                .addMenuItem({icon: 'scion.pin', label: 'Pin', onSelect})
                .addMenuItem({icon: 'scion.close', label: 'Close', onSelect: () => menuItems.update(menuItems => menuItems.filter(it => it !== menuItem))})
                .addGroup(group => group
                  .addMenuItem({icon: 'scion.chevron_up', label: 'Move up', onSelect})
                  .addMenuItem({icon: 'scion.chevron_down', label: 'Move down', onSelect}),
                )
                .addMenuItem({label: 'Close other tabs', onSelect})
                .addMenuItem({label: 'Close tabs below', onSelect}),
              )
              .addToolbarButton({
                icon: 'scion.close',
                tooltip: 'Close',
      ...
      .addGroup(group => group
        .addToolbarButton({icon: 'undo', tooltip: 'Undo', onSelect})
        .addToolbarButton({icon: 'redo', tooltip: 'Redo', onSelect})
        .addToolbarButton({icon: 'content_cut', tooltip: 'Cut', onSelect})
        .addToolbarButton({icon: 'content_copy', tooltip: 'Copy', onSelect})
        .addToolbarButton({icon: 'content_paste', tooltip: 'Paste', onSelect}),
      ),
    );
...
function contributeHeadingGroupActions(toolbar: SciToolbarFactory): void {
  toolbar
    ...
    .addToolbarMenu({icon: 'scion.scion.more_verticalical', visualMenuIndicator: false}, menu => menu
