...
@Component({
  ...
  animations: [
    trigger('grow-on-enter', provideGrowAnimation()),
  ],
})
...
function provideGrowAnimation(): AnimationMetadata[] {
  return [
    transition('* => enter', [
      style({width: 0, height: 0, position: 'absolute'}),
      sequence([
        animate('500ms', style({width: '300px', height: '300px'})),
        animate('500ms', style({width: '0', height: '0'})),
      ]),
    ]),
  ];
}
