...
export default class SciTreePageComponent {
  ...
  protected readonly activeItemId = computed(() => this.trees()[0]?.activeItem()?.id);
  protected readonly selectedItems = computed(() => this.trees()[0]?.selectedItems());
  protected readonly selection = computed(() => this.trees()[0]?.selectedItems().map(item => item.id).sort((a, b) => a - b).join(' '));

  constructor() {
    this.bindTreeSettings();
  }

  private createTree(options: {datasource: 'array' | 'array-http' | 'loader' | 'loader-delayed' | 'loader-http'; header: string; showNodeActions: boolean; customRowStyling: boolean; bufferSize: number; pageSize: number}): SciTree<Product> {
    return tree<Product>({
  ...
  private computeTrees(): Signal<SciTree<Product>[]> {
    ...
    effect(onCleanup => {
      ...
      const header = this.settingsForm.header().value();
      ...
      untracked(() => {
        ...
        trees.set(Array.from(Array(treeCount), (_, i) => runInInjectionContext(injector, () => this.createTree({datasource, header, showNodeActions: showNodeActions, customRowStyling, bufferSize, pageSize}))));
      });
    });
